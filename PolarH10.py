...
class PolarH10:
    ...
    def __new__(cls, bleak_device):
        # Si la instancia única no existe, créela
        if cls._instance is None:
            cls._instance = super(PolarH10, cls).__new__(cls)
            cls._instance.__initialized = False  # Añadimos un indicador de inicialización
    ...
    def hr_data_conv(self, sender, data):  
        ...
        print(f"energy_expenditure: {energy_expenditure}")
    ...
    async def print_device_info(self):
        ...
        self.deviceinformation_window.show()
        
        self.deviceinformation_window.label_modelnumber.setText(str(''.join(map(chr, self.model_number))))
        self.deviceinformation_window.label_manufacturerName.setText(str(''.join(map(chr, self.manufacturer_name))))
        self.deviceinformation_window.label_serialnumber.setText(str(''.join(map(chr, self.serial_number))))
        self.deviceinformation_window.label_address.setText(self.bleak_device.address)
        self.deviceinformation_window.progressBar.setValue(self.battery_level[0])
        self.deviceinformation_window.label_firmwarerevision.setText(str(''.join(map(chr, self.firmware_revision))))
        self.deviceinformation_window.label_hardwarerevision.setText(str(''.join(map(chr, self.hardware_revision))))
        self.deviceinformation_window.label_softwarerevision.setText(str(''.join(map(chr, self.software_revision))))
